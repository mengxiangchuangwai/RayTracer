...
static char* read_text_line( FILE* fp, char* buf, int maxlength )
{
	if( feof( fp ) )
		return NULL;
...
static void free_ply(PlyFile *plyfile )
{
	int i, j;
...
)
{
    ...
    for (i = 0; i < nelems; i++)
    {
        ...
        elem->name = (char* )strcopy (elem_names[i]);         /* added (char* ) cast 3/2/2005 */
        elem->num = 0;
        elem->nprops = 0;
    }
...
void ply_put_comment(PlyFile *plyfile, char *comment)
{
    ...
    /* add comment to list */
    plyfile->comments[plyfile->num_comments] = (char* )strcopy (comment);   /* added (char* ) cast 3/2/2005 */
...
void ply_put_obj_info(PlyFile *plyfile, char *obj_info)
{
    ...
    /* add info to list */
    plyfile->obj_info[plyfile->num_obj_info] = (char* )strcopy (obj_info);  /* added (char* ) cast 3/2/2005 */
...
//      elist[i] = (char* )strdup (plyfile->elems[i]->name);   /* added (char* ) cast 3/2/2005 */
        elist[i] = strcopy (plyfile->elems[i]->name);   /* added (char* ) cast 3/2/2005 */
    }
...
)
{
    ...
    other->name = (char* )strcopy (elem_name);    /* added (char* ) cast 3/2/2005 */
#if 0
...
)
{
    ...
    other->elem_name = (char* ) strcopy (elem_name);  /* added (char* ) cast 3/2/2005 */
...
//    result = fgets (str, BIG_STRING, fp);
	result = read_text_line( fp, str, BIG_STRING );
...
void add_element (PlyFile *plyfile, char **words, int nwords)
{
    ...
    elem = (PlyElement *) myalloc (sizeof (PlyElement));
    elem->name = (char* ) strcopy (words[1]);  /* added (char* ) cast 3/2/2005 */
...
void add_property (PlyFile *plyfile, char **words, int nwords)
{
    ...
    if (equal_strings (words[1], "list"))         /* is a list */
    {
        ...
        prop->name = (char* ) strcopy (words[4]);  /* added (char* ) cast 3/2/2005 */
        prop->is_list = 1;
    }
    else                                          /* not a list */
    {
        prop->external_type = get_prop_type (words[1]);
        prop->name = (char* ) strcopy (words[2]);  /* added (char* ) cast 3/2/2005 */
        prop->is_list = 0;
    }
...
{
    dest->name = (char* ) strcopy (src->name);
