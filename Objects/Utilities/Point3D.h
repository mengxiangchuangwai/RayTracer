...
class Point3D {
	...
	public:
		...
		DistanceSq(const Point3D& p) const;
		
		double														// distance bewteen two points
		Distance(const Point3D& p) const;

		void 
		Set(const double a, const double b, const double c);		// set the content of a point

		void
		Get( double& a, double& b, double& c) const;				// get the content of a point
...
inline double
Point3D::DistanceSq(const Point3D& p) const 
{
	return (	(x - p.x) * (x - p.x) 
			+ 	(y - p.y) * (y - p.y)
			+	(z - p.z) * (z - p.z) );
...
inline void
Point3D::Get( double& a, double& b, double& c) const 
{
	a = x;
	b = y;
	c = z;
